...
def combined_non_linear_encoding(x, t, params):
    qml.RY(mix_basis_encoding(x, params[0], params[1], params[2], params[3], params[4]), wires=0)
    qml.RZ(mix_basis_encoding(x, params[5], params[6], params[7], params[8], params[9]), wires=0)
    qml.RY(linear_encoding(t, params[10], params[11]), wires=0)
    qml.RZ(linear_encoding(t, params[12], params[13]), wires=0)
...
def def_loss(x_train, t_train, u_train, nlayers, encoding_type):
    ...
    def rnd_param_init(seed=None):
        if seed is not None:
            np.random.seed(seed)
        params = qml.numpy.array(np.random.uniform(low=0, high=2 * np.pi, size=(nlayers, 14), requires_grad=True))

        params[:, :10] = (params[:, :10] - np.pi) / 100
        params[:,12] = (params[:,6] - np.pi) / 100
        return params
...
def main():
    ...
    save_path = f"../results_diff_enc_50/{encoding}Enc_{nlayers}layers_{npoints}data_{epochs}epochs_{lr}lr_{random_seed}rng"
