...
def scaled_softsign_encoding(x, a, b):
    """
    f(x) = π * (a * x + b) / (1 + |a * x + b|)
    """
    z = a * x + b
    return np.pi * z / (1 + np.abs(z))

def scaled_sigmoid_encoding(x, a):
    """
    f(x) = π / (1 + exp(-a * x))
    """
    return np.pi / (1 + np.exp(-a * x))

def cap_encoding(x, a, b):
    """
    f(x) = clip(a * x + b, 0, π)
    """
    return np.clip(a * x + b, 0, np.pi)
...
def logistic_encoding(x, a, b, k):
    """
    f(x) = a * x + b * (1 / (1 + exp(k * x))) * (1 - x)
    """
    logistic = 1 / (1 + np.exp(k * x))
    return a * x + b * logistic * (1 - x)
